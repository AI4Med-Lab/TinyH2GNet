...
from utils.dataPrep_locigen import PatchDataset, compute_gene_stats
from utils.loss_lociGene import locigen_loss, locigen_loss_v2_1
...
from lociGen import LoCIGENv2, LoCIGEN_v2_2, LoCIGEN_v3, LoCIGEN_MIL, LoCIGEN_v3_Neighborhood
...
def main():
    ...
    def train_regression(train_datasets, val_datasets, fold_idx, g, seed_worker,
        ...
        elif params["model"] == "LoCIGEN":
            logger.info("Initializing LoCIGEN model...")
            pretrained = params.get("pretrained", True)
            model = LoCIGENv2(num_genes=num_genes, pretrained=pretrained, dropout=0.3)
            # model = LoCIGEN_v2_2( num_genes=171, backbone_name="efficientnet_b0",)
            # model = LoCIGEN_v3( num_genes=num_genes, pretrained=pretrained)
            
            # model = LoCIGEN_v3_Neighborhood( num_genes=num_genes, pretrained=params.get("pretrained", True))
        
        elif params["model"] == "LoCIGEN_MIL":
            logger.info("Initializing LoCIGEN MIL model...")
            pretrained = params.get("pretrained", True)
            model = LoCIGEN_MIL( num_genes=num_genes, pretrained=pretrained)
        ...
        elif params.get("optimizer", "Adamw").lower() == "adamw":   
            optimizer = torch.optim.AdamW(
                model.parameters(),
                lr=learning_rate,
                weight_decay=weight_decay
            )
        # this was used in case of locigen v3
        # elif params.get("optimizer", "Adamw").lower() == "adamw":
        #     optimizer = torch.optim.AdamW([
        #         {"params": model.backbone.parameters(), "lr": 2e-5},
        #         {"params": model.coord_encoder.parameters(), "lr": 5e-4},
        #         {"params": model.cond_proj.parameters(), "lr": 5e-4},
        #         {"params": model.gene_embed.parameters(), "lr": 5e-4},
        #         {"params": model.gene_blocks.parameters(), "lr": 5e-4},
        #         {"params": model.neigh_smoother.parameters(), "lr": 5e-4},
        #         {"params": model.expr_head.parameters(), "lr": 5e-4},
        #         {"params": model.log_var_head.parameters(), "lr": 5e-4},
        #     ])
        # this was used in case of locigen mil version
        # elif params.get("optimizer", "Adamw").lower() == "adamw":
        #     optimizer = torch.optim.AdamW([
        #         # CNN backbone (slow LR)
        #         {"params": model.backbone.parameters(), "lr": learning_rate},

        #         # Instance projection
        #         {"params": model.inst_proj.parameters(), "lr": 5e-4},
        ...
        #         {"params": model.gene_attn.parameters(), "lr": 5e-4},
        ...
        gene_mean, gene_std = compute_gene_stats(train_dataloader)

        gene_mean = gene_mean.to(device)
        gene_std = gene_std.to(device)

        # Training loop
        for epoch in range(epochs):
            # UNFREEZE_EPOCH = params.get('UNFREEZE_EPOCH', 10)   # you can tune this (10–15 works well)

            # if epoch == UNFREEZE_EPOCH:
            #     for g in optimizer.param_groups:
            #         # backbone group has lr = 1e-5 initially
            #         if abs(g["lr"] - 1e-5) < 1e-10:
            #             g["lr"] = 3e-5
            #     logger.info(f"Epoch {epoch}: Backbone LR increased to 3e-5")
            ...
            for batch_idx, (images, coords, targets) in enumerate(train_dataloader):
                # Move data to device
                ...
                elif loss_fn_name == "locigen_loss":
                    loss = locigen_loss(out, y_true)
                elif loss_fn_name == "locigen_loss_v2":
                    loss = locigen_loss_v2_1(out, y_true, gene_mean, gene_std)
                else:
            ...
            # gene_mean, gene_std = compute_gene_stats(val_dataloader)

            # gene_mean = gene_mean.to(device)
            # gene_std = gene_std.to(device)

            with torch.no_grad():
                for batch_idx, (images, coords, targets) in enumerate(val_dataloader):
