...
def main():
    ...
    def train_regression(train_datasets, val_datasets, fold_idx,
        ...
        if params["model"] == "STNet":
            ...
            pretrained = params.get("pretrained", False)
            model = EfficientNetB4GeneRegressor(num_genes=num_genes, pretrained=pretrained) 
        ...
        elif params["model"] == "ImageGeneCrossTransformerWithSinePE":
            # --- Get model hyperparameters ---
            embed_dim = params.get("embed_dim", 512)
            nhead = params.get("nhead", 8)
            dim_feedforward = params.get("dim_feedforward", 1024)
            ...
            # --- Logging configuration ---
            logger.info("Initializing ImageGeneCrossTransformerWithSinePE with:")
            logger.info(f"- Image size: {H}x{W}")
            logger.info(f"- Image channels: {image_channels}")
            logger.info(f"- Embedding dimension (d_model): {embed_dim}")
            logger.info(f"- Attention heads: {nhead}")
            ...
            # --- Model initialization ---
            model = ImageGeneCrossTransformerWithSinePE(
                num_genes=num_genes,
                d_model=embed_dim,
                nhead=nhead,
                dim_feedforward=dim_feedforward,
        ...
        # ImageGeneCrossTransformerWithSinePE
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (images, targets, x, y) in enumerate(train_dataloader):
                # Move data to device
                images = images.to(device)             # (B, 3, 224, 224)
                y_true = targets.to(device)            # (B, num_genes)
                x = x.to(device)
                y = y.to(device)

                # Linearly schedule dropout
                conditioning_dropout = min(1.0, initial_dropout + (final_dropout - initial_dropout) * (epoch / decay_epochs))
                logger.info(f"Epoch {epoch + 1}/{epochs} - Conditioning dropout: {conditioning_dropout:.3f}")
                ...
                if params["model"] in ["ImageGeneCrossTransformer", "ImageToGeneTransformer","ImageGeneCrossTransformerWithSinePE", "ImageGeneCrossTransformerRoPE"]:
                    if np.random.rand() < conditioning_dropout:
                        ...
                        logger.info(f"mode used:: {mode_used}")
                    else:
                        output = model(images, gene_values=y_true)  # teacher-forced
                        mode_used = "teacher_forced"
                        logger.info(f"mode used:: {mode_used}")
                else:
                    output = model(images)
                    mode_used = "baseline"
                ...
                # Inspect the output type/shape
                if isinstance(output, tuple):
                    print(f"\n[DEBUG] Model returned a tuple of length {len(output)} in mode={mode_used}")
                    for i, elem in enumerate(output):
                        if torch.is_tensor(elem):
                            print(f"  elem[{i}] shape={tuple(elem.shape)} dtype={elem.dtype}")
                        else:
                            print(f"  elem[{i}] type={type(elem)}")
                    y_pred = output[0]  # take the prediction tensor
                else:
                    print(f"\n[DEBUG] Model returned a tensor shape={tuple(output.shape)} in mode={mode_used}")
                    y_pred = output

                print(f"y_pred type: {type(y_pred)}")

                # Clamp negatives (safer)
                y_pred = torch.clamp(y_pred, min=0.0)

                print(f"images: {images.shape}, y_true: {y_true.shape}, y_pred: {y_pred.shape}")
                if batch_idx == 0:
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(range(n_samples))):
        ...
        initial_dropout = params.get("conditioning_dropout", 0.5)
        final_dropout = 1.0  # inference-only mode at the end
        decay_epochs = params.get("conditioning_decay_epochs", int(params["epochs"]) // 2)
