...
def load_model(model_name, model_path, num_genes, quantize=False, save_quantized_path=None):
    ...
    if quantize:
        print("[INFO] Applying INT8 post-training quantization")
        model.to("cpu")
        model.eval()
        ...
        if save_quantized_path is not None:
            torch.save(model, save_quantized_path)
            print("[INFO] Quantized model saved at:", save_quantized_path)
...
def evaluate(params):
    ...
    if params["dataset_name"] == "hist2st_845":
        ...
        mapping_csv = "/home/puneet/mk/code_data_preprocessing/hvg/hvg_lists/his2st_geneid_to_gene_all.csv"
    ...
    for fold, (_, val_idx) in enumerate(kf.split(gsm_samples, groups=patient_ids)):
        ...
        model = load_model(
            ...
            save_quantized_path=os.path.join(save_root, "model_int8.pth")
        )
        ...
        if DEVICE.type == "cuda":
            ...
            cpu_mem_delta = cpu_mem_after - mem_before
            peak_mem_alloc = None
        ...
        if DEVICE.type == "cuda":
            ...
            print(f"[Fold {fold+1}] Inference time: {fold_time:.2f}s | "
                f"Throughput: {throughput:.2f} spots/sec | "
                f"CPU RAM: {cpu_mem_after:.1f} MB (Δ {cpu_mem_delta:.1f} MB)")
        ...
        FOLD_TIMINGS.append({
            ...
            "cpu_mem_after_mb": cpu_mem_after,
            "cpu_mem_delta_mb": cpu_mem_delta,
    ...
    if "peak_gpu_mem_allocated_mb" in timing_df:
        summary["mean_peak_gpu_mem_alloc_mb"] = timing_df["peak_gpu_mem_allocated_mb"].mean()
        summary["std_peak_gpu_mem_alloc_mb"] = timing_df["peak_gpu_mem_allocated_mb"].std()

    if "cpu_mem_after_mb" in timing_df:
        summary["mean_cpu_mem_after_mb"] = timing_df["cpu_mem_after_mb"].mean()
        summary["std_cpu_mem_after_mb"] = timing_df["cpu_mem_after_mb"].std()
