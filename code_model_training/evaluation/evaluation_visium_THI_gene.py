...
from models import STNet, EfficientNet, EfficientNetB4GeneRegressor, Custom_VGG16, HisToGene, TCGN, EfficientNet_GeneCaptionContrastive, MobileNetV2Regressor
from models import RefinedTinySTNet, RefinedTinyEfficientNet, THItoGeneModel
...
def load_model(model_name, model_path, num_genes):
    if model_name == "EfficientNet":
    ...
    elif model_name == "ReTinySTNet":
        model = RefinedTinySTNet(num_genes=num_genes)  # student model only
    
    elif params["model"] == "EfficientNetTinyStudent":
        print("EfficientNetTinyStudent (compound-scaled) for gene expression prediction.")
        ...
        print(f"RefinedTinyEfficientNet model for gene expression prediction.")
        ...
        print(f"TCGN initialized with {sum(p.numel() for p in model.parameters() if p.requires_grad):,} trainable parameters.")
            ...
            model = MobileNetV2Regressor(num_genes=num_genes)
            
    elif params["model"] == "THItoGene":
        print("Initializing THItoGene model for gene expression prediction.")
        ...
        print(f"THItoGene initialized with {sum(p.numel() for p in model.parameters() if p.requires_grad):,} trainable parameters.")
...
def predict(model, dataset):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            gts.append(y_true_2d.detach().cpu().numpy())
            preds.append(y_pred_2d.detach().cpu().numpy())
            
            # preds.append(y_pred.cpu().numpy())
            # gts.append(y_true.cpu().numpy())
...
def evaluate(params):
    ...
    if params["dataset_name"] == "hist2st_845":
        ...
        mapping_csv = "/home/puneet/mk/code_dataprocessing/hvg/hvg_lists/his2st_geneid_to_gene_all.csv"
    ...
    for fold, (_, val_idx) in enumerate(kf.split(gsm_samples, groups=patient_ids)):
        ...
        if params["model"] == "THItoGene" and params["dataset_name"] == "hist2st_845":
            
            print("hist2st thitogene")
        ...
        elif params["model"] == "THItoGene" and params["dataset_name"] == "her2":
            
            val_datasets_list = [
                THItoGeneHER2Dataset(
                    h5_img_path=img_p,
                    h5ad_path=adata_p,
                    gene_names=gene_names,
                    transform=transform,  # same resize for val (no aug)
